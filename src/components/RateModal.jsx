...
const RateModal = ({ open, handler, manuscriptData, userData }) => {
  ...
  const handleBTN = () => {
    ...
    rateManuscript(
      manuscriptData.id,
      manuscriptData.location,
      manuscriptData.title,
      rating,
      userData.uid,
      userData.displayName,
      manuscriptData.frontPageURL,
      manuscriptData.abstract,
      manuscriptData.department
    );
  };
  useEffect(() => {
    setRated(manuscriptData.rated);
  }, [manuscriptData]);
  const rateManuscript = async (
    manuscriptId,
    manuscriptLocation,
    manuscriptName,
    rating,
    ...
    try {
      ...
      if (1 === rating) {
        const docUpdate = await updateDoc(manuscriptRef, {
          rated: {
            ...rated,
            five: rated.five || 0,
            four: rated.four || 0,
  ...
  return (
    <Dialog open={open} size={"xs"}>
      ...
      <DialogFooter className="flex gap-x-2 justify-center">
        <Button color="amber" onClick={handleBTN}>
          Rate
        </Button>
